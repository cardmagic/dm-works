...
class ActsAsSolrTest < Test::Unit::TestCase
  ...
  def test_index_and_search_with_html_entities
    ...
    posting = Posting.create(:guid => 'ASD', :name => "Test in Action&trade; - Copyright &copy; Bob", :description => description)
    assert posting.valid?

    postings = Posting.find_by_solr 'trademark &copy &#190 &iexcl &#163'
    assert_equal 1, postings.total
  end
